...
export default function GroupLoanScreen({ navigation }: any) {
  ...
  const loadLoans = async () => {
    try {
      setLoading(true);
      const data = await loanApi.getLoans(activeChamaId!);
      setLoans(data);
    } catch (err) {
      console.error(err);
    } finally {
  ...
  const pendingLoans = loans.filter((l: any) => l.status === "pending_vote");
  const activeLoans = loans.filter((l: any) => l.status === "active");
  const historyLoans = loans.filter((l: any) =>
    ["completed", "defaulted"].includes(l.status),
  );
      ...
      <View style={S.tabRow}>
        {TABS.map((t) => (
          <Pressable
            ...
            style={[
              S.tabBtn,
              tab === t && { ...S.tabBtnActive, backgroundColor: themeColor },
            ]}
            onPress={() => setTab(t)}
          >
      ...
      >
        ...
        {(tab === "Active" || tab === "Vote") &&
          pendingLoans.map((loan) => (
            <View style={S.voteCard} key={loan.id}>
              <Text style={S.votePending}>Pending vote</Text>
              <Text style={S.voteRequestor}>
                {loan.borrowerMember?.user?.fullName || "Member"} requests
              </Text>
              <Text style={S.voteAmount}>
                Ksh {Number(loan.amount).toLocaleString()}
              </Text>
              <Text style={S.votePurpose}>
                Purpose: {loan.purpose} · {loan.repaymentMonths} months
              </Text>

              <View style={S.voteStats}>
                <View style={S.voteStat}>
                  <Text style={S.voteStatNum}>
                    {loan.votes?.filter((v: any) => v.vote === "yes").length ||
                      0}
                  </Text>
                  <Text style={[S.voteStatLabel, { color: "#059669" }]}>
                    Yes
                  </Text>
                </View>
                <View style={S.voteStat}>
                  <Text style={[S.voteStatNum, { color: "#DC2626" }]}>
                    {loan.votes?.filter((v: any) => v.vote === "no").length ||
                      0}
                  </Text>
                  <Text style={[S.voteStatLabel, { color: "#DC2626" }]}>
                    No
                  </Text>
                </View>
                <View style={S.voteStat}>
                  <Text style={S.voteStatNum}>-</Text>
              ...
              <View style={S.voteActions}>
                <TouchableOpacity
                  ...
                  activeOpacity={0.85}
                  onPress={async () => {
                    await loanApi.voteLoan(activeChamaId!, loan.id, "yes");
                    loadLoans();
                  }}
                >
                ...
                <TouchableOpacity
                  ...
                  activeOpacity={0.85}
                  onPress={async () => {
                    await loanApi.voteLoan(activeChamaId!, loan.id, "no");
                    loadLoans();
                  }}
                >
            ...
            {[
              ...
              <View key={i} style={S.activeLoanCard}>
                <View style={S.activeLoanTop}>
                  ...
                  <Text
                    style={[
                      S.activeLoanAmtLarge,
                      { fontSize: 15, color: loan.color },
                    ]}
                  >
                    {loan.amount}
                  </Text>
