...
function AnimatedToggle({ isOn }: { isOn: boolean }) {
  ...
  const trackColor = anim.interpolate({
    inputRange: [0, 1],
    outputRange: ["#C8D8D4", Colors.primary],
  });
  ...
  return (
    <View style={S.hybridWrap}>
      ...
      <Text style={S.hybridSub}>
        Choose which components and how to split contributions
      </Text>

      <Text style={[S.sliderLabel, { marginTop: 4, marginBottom: 8 }]}>
        CHAMA NAME
      </Text>
      ...
      <View style={S.toggleBlock}>
        <View style={S.toggleRow}>
          <View style={[S.toggleIcon, { backgroundColor: Colors.surfaceElevated }]}>
            <Feather name="rotate-cw" size={16} color="#D97706" />
          </View>
          <Text style={S.toggleName}>Merry-go-round</Text>
          <View style={S.alwaysOnBadge}>
        ...
        {investEnabled && (
          <View style={{ marginTop: 12 }}>
            <View style={S.autoBarHeader}>
              ...
              <Text style={[S.autoBarPct, { color: Colors.primary }]}>
                {investPct}%
              </Text>
            </View>
            <View style={S.autoTrack}>
              <View
                style={[
                  S.autoFill,
                  { width: `${investPct}%` as any, backgroundColor: "#93C5FD" },
                ]}
              />
            </View>
          </View>
      ...
      <View style={S.breakdownCard}>
        ...
        <View style={S.breakdownRows}>
          <View style={S.breakdownRow}>
            <View style={[S.bDot, { backgroundColor: Colors.background }]} />
        ...
        {!welfareEnabled && (
          <Text style={S.breakdownNote}>
            No welfare pot yet — toggle it above to add one
          </Text>
        )}
      </View>
    </View>
...
const TYPE_ICON_BG: Record<string, string> = {
  merry_go_round: Colors.surfaceElevated,
  investment: Colors.surfaceElevated,
  welfare: "#EDE9FE",
  hybrid: "#D1FAE5",
  group_purchase: Colors.surfaceElevated,
};
const TYPE_FEATHER: Record<
  string,
...
const CHAMA_TYPES = [
  {
    id: "merry_go_round" as ChamaTypeId,
    title: "Merry-go-round",
    description: "Pot rotates to one member per cycle until all have received.",
    badge: "MOST POPULAR",
    bestFor: "Predictable lump-sum payouts",
  },
  {
    id: "investment" as ChamaTypeId,
    title: "Investment",
    description: "Group votes on stocks, land, unit trusts, or a business.",
    bestFor: "Long-term wealth building",
  },
  {
    id: "welfare" as ChamaTypeId,
    title: "Welfare / savings",
    description: "Borrow from the group pot at low interest.",
    bestFor: "Emergency affordable credit",
  },
  {
    id: "hybrid" as ChamaTypeId,
    title: "Hybrid",
    description: "Combine two or three types with a custom split.",
    bestFor: "Maximum flexibility",
  },
  {
    id: "group_purchase" as ChamaTypeId,
    title: "Group purchase",
    description:
      "Members save together to buy the same product for every household — one per cycle.",
    bestFor: "Appliances, furniture, solar, insurance",
  },
];
    ...
    >
      <Animated.View style={[S.overlay, { opacity: fade }]}>
        ...
        >
          ...
          <Text style={S.nudgeQ}>
            Advice: Choosing MGR alone is good, but we advise you to pair it
            with a savings pot so that the chama can have a mini-bank just in
            case a member needs some urgent money before their turn.
          </Text>
          ...
          <View style={S.scenarioRow}>
            ...
            <View style={[S.scenario, S.scenarioGood]}>
              ...
              <Text style={S.scenarioGoodText}>
                Borrows from group in 48 hrs at 5%.
              </Text>
            </View>
          </View>
      ...
      >
        ...
        <View style={S.chairNote}>
          ...
          <Text style={S.chairText}>
            Creating this chama makes you its{" "}
            <Text style={S.chairBold}>Chairperson</Text> automatically.
          </Text>
        </View>
      </ScrollView>
...
const S = StyleSheet.create({
  ...
  // Hybrid builder
  hybridWrap: { padding: 14, backgroundColor: Colors.surface },
  hybridTitle: {
    fontFamily: FontFamily.heading,
    fontSize: 13,
    color: "#E8D6B5",
  ...
  overlay: {
    flex: 1,
    backgroundColor: "rgba(7,21,16,0.65)",
  ...
  scenarioBad: { backgroundColor: Colors.surface, borderColor: Colors.border },
  scenarioGood: { backgroundColor: Colors.surface, borderColor: Colors.border },
  scenarioBadLabel: {
    fontFamily: FontFamily.heading,
    fontSize: 12,
    color: "#DC2626",
